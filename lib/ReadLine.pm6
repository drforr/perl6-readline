...
#sub rl_tty_set_default_bindings (Keymap $kmap)
#  is native( LIB ) { ... }
...
class ReadLine {
  method readline( Str $prompt ) { readline( $prompt ) }
  method add-history( $prompt ) { add_history( $prompt ) }

  # 2.4.3
  #
  method unbind-key( Int $key ) { rl_unbind_key( $key ) }
  method parse-and-bind( Str $line ) { rl_parse_and_bind( $line ) }
  method read-init-file( Str $filename ) { rl_read_init_file( $filename ) }

  # 2.4.4
  #
  method function-dumper( Int $readable ) { rl_function_dumper( $readable ) }
  method list-funmap-names( ) { rl_list_funmap_names( ) }

  # 2.4.5
  #
  method begin-undo-group( ) { rl_begin_undo_group( ) }
  method end-undo-group( ) { rl_end_undo_group( ) }
  method free-undo-list( ) { rl_free_undo_list( ) }
  method do-undo( ) { rl_do_undo( ) }
  method modifying( Int $start, Int $end ) { rl_modifying( $start, $end ) }

  # 2.4.6
  #
  method redisplay( ) { rl_redisplay( ) }
  method forced-update-display( ) { rl_forced_update_display( ) }
  method on-new-line( ) { rl_on_new_line( ) }
  method on-new-line-with-prompt( ) { rl_on_new_line_with_prompt( ) }
  method reset-line-state( ) { rl_reset_line_state( ) }
  method crlf( ) { rl_crlf( ) }
  method show-char( Int $c ) { rl_show_char( $c ) }
  method clear-message( ) { rl_clear_message( ) }
  method restore-prompt( ) { rl_restore_prompt( ) }
  method expand-prompt( Str $prompt ) { rl_expand_prompt( $prompt ) }
  method set-prompt( Str $prompt ) { rl_set_prompt( $prompt ) }

  # 2.4.7
  #
  method insert-text( Str $text ) { rl_insert_text( $text ) }
  method delete-text( Int $start, Int $end ) { rl_delete_text( $start, $end ) }
  method copy-text( Int $start, Int $end ) { rl_copy_text( $start, $end ) }
  method kill-text( Int $start, Int $end ) { rl_kill_text( $start, $end ) }
  method push-macro-input( Str $macro ) { rl_push_macro_input( $macro ) }

  # 2.4.8
  #
  method read-key( ) { rl_read_key( ) }
  method stuff-char( Int $c ) { rl_stuff_char( $c ) }
  method execute-next( Int $c ) { rl_execute_next( $c ) }
  method clear-pending-input( ) { rl_clear_pending_input( ) }
  method set-keyboard-input-timeout( Int $u ) {
    rl_set_keyboard_input_timeout( $u ) }

  # 2.4.9
  #
  method prep-terminal( Int $meta_flag ) { rl_prep_terminal( $meta_flag ) }
  method deprep-terminal( ) { rl_deprep_terminal( ) }
  method reset-terminal( Str $terminal_name ) {
    rl_reset_terminal( $terminal_name ) }

  # 2.4.10
  #
  method replace-line( Str $text, Int $clear_undo ) {
    rl_replace_line( $text, $clear_undo ) }
  method extend-line-buffer( Int $len ) { rl_extend_line_buffer( $len ) }
  method initialize( ) { rl_initialize( ) }
  method ding( ) { rl_ding( ) }
  method alphabetic( Int $c ) { rl_alphabetic( $c ) }

  # 2.4.11
  #
  method macro-dumper( Int $readable ) { rl_macro_dumper( $readable ) }
  method variable-bind( Str $variable, Str $value ) {
    rl_variable_bind( $variable, $value ) }
  method variable-value( Str $variable ) { rl_variable_value( $variable ) }
  method variable-dumper( Int $readable ) { rl_variable_dumper( $readable ) }
  method set-paren-blink-timeout( Int $u ) { rl_set_paren_blink_timeout( $u ) }
  method get-termcap( Str $cap ) { rl_get_termcap( $cap ) }
  method clear-history( ) { rl_clear_history( ) }

  # 2.4.12
  #
  method callback-read-char( ) { rl_callback_read_char( ) }
  method callback-handler-remove( ) { rl_callback_handler_remove( ) }

  # 2.5
  #
  method cleanup-after-signal( ) { rl_cleanup_after_signal( ) }
  method free-line-state( ) { rl_free_line_state( ) }
  method reset-after-signal( ) { rl_reset_after_signal( ) }
  method echo-signal-char( Int $sig ) { rl_echo_signal_char( $sig ) }
  method resize-terminal( ) { rl_resize_terminal( ) }
  method set-screen-size( Int $rows, Int $cols ) {
    rl_set_screen_size( $rows, $cols ) }
  method reset-screen-size( ) { rl_reset_screen_size( ) }
  method clear-signals( ) { rl_clear_signals( ) }

  # 2.6.2
  #
  method complete-internal( Int $what_to_do ) {
    rl_complete_internal( $what_to_do ) }
  method complete( Int $ignore, Int $invoking_key ) {
    rl_complete( $ignore, $invoking_key ) }
  method possible-completions( Int $count, Int $invoking_key ) {
    rl_possible_completions( $count, $invoking_key ) }
  method insert-completions( Int $count, Int $invoking_key ) {
    rl_insert_completions( $count, $invoking_key ) }
