...
=item replace-history-entry( Int $which, Str $line, histdata_t $data )
...
=item rl-bind-key-in-map( Int $i, rl_command_func_t $cb, Keymap $k )
...
=item rl-bind-key-if-unbound-in-map
...
=item rl-unbind-function-in-map ( rl_command_func_t $cb, Keymap $k )

    Unbind all keys that execute function in map. 

=item rl-unbind-command-in-map ( Str $str, Keymap $k )

    Unbind all keys that are bound to command in map. 

=item rl-bind-keyseq( Str $str, rl_command_func_t $cb )
...
=item rl-bind-keyseq-if-unbound( Str $str, rl_command_func_t $cb )
...
=item rl-bind-keyseq-if-unbound-in-map
...
=item rl-function-of-keyseq( Str $s, Keymap $k, Pointer[Int] $p )
...
class ReadLine {
  ...
  sub rl_unbind_command_in_map( Str, Keymap ) returns Int
    is native( LIB ) { * }
  method rl-unbind-command-in-map ( Str $str, Keymap $k )
    returns Int {
      rl_unbind_command_in_map( $str, $k ) }
  ...
  sub rl_invoking_keyseqs_in_map( Pointer[rl_command_func_t], Keymap ) returns Pointer[Str]
    is native( LIB ) { * }
  method rl-invoking-keyseqs-in-map( Pointer[rl_command_func_t] $p-cmd, Keymap $k ) returns Pointer[Str] {
    rl_invoking_keyseqs_in_map( $p-cmd, $k ) }

  sub rl_invoking_keyseqs( Pointer[rl_command_func_t] ) returns Pointer[Str]
    is native( LIB ) { * }
  method rl-invoking-keyseqs( Pointer[rl_command_func_t] $p-cmd ) returns Pointer[Str] {
    rl_invoking_keyseqs( $p-cmd ) }
  ...
  sub rl_reset_line_state( ) returns Int
    is native( LIB ) { * }
  method rl-reset-line-state( ) returns Int {
    rl_reset_line_state( ) }
  ...
  sub rl_username_completion_function ( Str, Int ) returns Str
    is native( LIB ) { * }
  method rl-username-completion-function ( Str $s, Int $i ) returns Str {
    rl_username_completion_function ( $s, $i ) }

  sub rl_filename_completion_function ( Str, Int ) returns Str
    is native( LIB ) { * }
  method rl-filename-completion-function ( Str $s, Int $i ) returns Str {
    rl_filename_completion_function ( $s, $i ) }

  sub rl_completion_mode( Pointer[rl_command_func_t] $cb ) returns Int
    is native( LIB ) { * }
  method rl-completion-mode( Pointer[rl_command_func_t] $cb ) returns Int {
