...
class ReadLine {
  ...
  class Keymap is repr('CPointer') {
    ...
    #
    sub rl_copy_keymap ( Keymap $keymap ) returns Keymap
      is native( LIB ) { * }
...
#typedef int rl_command_func_t PARAMS((int, int));
  ...
  # map.
  # 
  sub rl_generic_bind ( Str $type, Str $keyseq, Str $data, Keymap $map )
    returns Int
    is native( LIB ) { * }
  ...
  # bindings and variable assignments found (see section 1.3 Readline Init
  # File).
  # 
  ...
  # Read keybindings and variable assignments from filename (see section 1.3
  # Readline Init File).
  # 
  ...
  # Return the function invoked by keyseq in keymap map. If map is NULL, the
  # current keymap is used. If type is not NULL, the type of the object is
  # returned in the int variable it points to (one of ISFUNC, ISKMAP, or
  # ISMACR).
  # 
  #sub rl_command_func_t * rl_function_of_keyseq ( Str $keyseq, Keymap $map, int *type)
  #  is native( LIB ) { * }

  # Return an array of strings representing the key sequences used to invoke
  ...
  # Print the readline function names and the key sequences currently bound to
  # them to rl_outstream. If readable is non-zero, the list is formatted in
  # such a way that it can be made part of an inputrc file and re-read.
  # 
  ...
  # Begins saving undo information in a group construct. The undo information
  # usually comes from calls to rl_insert_text() and rl_delete_text(), but
  # could be the result of calls to rl_add_undo().
  # 
  ...
  # Closes the current undo group started with rl_begin_undo_group (). There
  # should be one call to rl_end_undo_group() for each call to
  # rl_begin_undo_group().
  # 
  ...
  # from start to end, and encompasses text.
  # 
  ...
  # Undo the first thing on the undo list. Returns 0 if there was nothing to
  # undo, non-zero if something was undone.
  # 
  ...
  # Tell Readline to save the text between start and end as a single undo unit.
  ...
  # rl_prompt already displayed. This could be used by applications that want
  # to output the prompt string themselves, but still need Readline to know the
  # prompt string length for redisplay. It should be used after setting
  # rl_already_prompted.
  # 
  ...
  # Display character c on rl_outstream. If Readline has not been set to
  ...
  # The arguments are a format string as would be supplied to printf, possibly
  # containing conversion specifications such as `%d', and any additional
  # arguments necessary to satisfy the conversion specifications. The resulting
  # string is displayed in the echo area. The echo area is also used to display
  # numeric arguments and search strings. You should call rl_save_prompt to
  ...
  # rl_save_prompt before the last call to rl_message, call rl_restore_prompt
  ...
  # Save the local Readline prompt display state in preparation for displaying
  # a new message in the message area with rl_message().
  # 
  sub rl_save_prompt ( )
    is native( LIB ) { * }
  method save-prompt( ) {
  ...
  # Restore the local Readline prompt display state saved by the most recent
  # call to rl_save_prompt. if rl_save_prompt was called to save the prompt
  # before a call to rl_message, this function should be called before the
  ...
  # Expand any special character sequences in prompt and set up the local
  # Readline prompt redisplay variables. This function is called by readline().
  # It may also be called to expand the primary prompt if the
  # rl_on_new_line_with_prompt() function or rl_already_prompted variable is
  # used. It returns the number of visible characters on the last line of the
  # (possibly multi-line) prompt. Applications may indicate that the prompt
  # contains characters that take up no physical screen space when displayed by
  # bracketing a sequence of such characters with the special markers
  # RL_PROMPT_START_IGNORE and RL_PROMPT_END_IGNORE (declared in `readline.h'.
  ...
  # rl_expand_prompt() to expand the prompt and sets rl_prompt to the result.
  # 
  ...
  # Cause macro to be inserted into the line, as if it had been invoked by a
  # key bound to a macro. Not especially useful; use rl_insert_text() instead.
  # 
  ...
  # This handles input inserted into the input stream via rl_pending_input
  # (see section 2.3 Readline Variables) and rl_stuff_char(), macros, and
  # characters read from the keyboard. While waiting for input, this function
  # will call any function assigned to the rl_event_hook variable.
  # 
  ...
  # Return the next character available from stream, which is assumed to be the
  # keyboard.
  # 
  #sub rl_getc (FILE *stream) returns Int
  #  is native( LIB ) { * }

  # Insert c into the Readline input stream. It will be "read" before Readline
  # attempts to read characters from the terminal with rl_read_key(). Up to 512
  # characters may be pushed back. rl_stuff_char returns 1 if the character was
  ...
  # Make c be the next command to be executed when rl_read_key() is called.
  ...
  # Unset rl_pending_input, effectively negating the effect of any previous
  # call to rl_execute_next(). This works only if the pending input has not
  # already been read with rl_read_key().
  # 
  ...
  # While waiting for keyboard input in rl_read_key(), Readline will wait for
  # u microseconds for input before calling any function assigned to
  # rl_event_hook. u must be greater than or equal to zero (a zero-length
  # timeout is equivalent to a poll). The default waiting period is one-tenth
  # of a second. Returns the old timeout value.
  # 
  ...
  # single character at a time from the keyboard. The meta_flag argument should
  # be non-zero if Readline should read eight-bit input.
  # 
  ...
  # Undo the effects of rl_prep_terminal(), leaving the terminal in the state
  # in which it was before the most recent call to rl_prep_terminal().
  # 
  ...
  # Reset the bindings manipulated by rl_tty_set_default_bindings so that the
  # terminal editing characters are bound to rl_insert. The bindings are
  # performed in kmap. 
  #
  ...
  # Save a snapshot of Readline's internal state to sp. The contents of the
  # readline_state structure are documented in `readline.h'. The caller is
  ...
  # Restore Readline's internal state to that stored in sp, which must have
  # been saved by a call to rl_save_state. The contents of the readline_state
  # structure are documented in `readline.h'. The caller is responsible for
  ...
  # Ensure that rl_line_buffer has enough space to hold len characters,
  ...
  # A convenience function for displaying a list of strings in columnar format
  # on Readline's output stream. matches is the list of strings, in argv format,
  # such as a list of completion matches. len is the number of strings in
  # matches, and max is the length of the longest string in matches. This
  # function uses the setting of print-completions-horizontally to select how
  # the matches are displayed (see section 1.3.1 Readline Init File Syntax).
  # When displaying completions, this function sets the number of columns used
  # for display to the value of completion-display-width, the value of the
  # environment variable COLUMNS, or the screen width, in that order. 
  #
  ...
  # Bind the key sequence keyseq to invoke the macro macro. The binding is
  # performed in map. When keyseq is invoked, the macro will be inserted into
  # the line. This function is deprecated; use rl_generic_bind() instead.
  # 
  ...
  # Print the key sequences bound to macros and their values, using the current
  # keymap, to rl_outstream. If readable is non-zero, the list is formatted in
  # such a way that it can be made part of an inputrc file and re-read.
  # 
  ...
  # (see section 1.3.1 Readline Init File Syntax).
  # 
  ...
  # If readable is non-zero, the list is formatted in such a way that it can be
  # made part of an inputrc file and re-read.
  # 
  ...
  # Set the time interval (in microseconds) that Readline waits when showing a
  # balancing character when blink-matching-paren has been enabled.
  # 
  ...
  # of prompt. Save the value of lhandler to use as a handler function to call
  # when a complete line of input has been entered. The handler function
  # receives the text of the line as an argument.
  # 
  #sub rl_callback_handler_install ( Str $prompt, rl_vcpfunc_t *lhandler)
  #  is native( LIB ) { * }

  # Whenever an application determines that keyboard input is available, it
  # should call rl_callback_read_char(), which will read the next character
  # from the current input source. If that character completes the line,
  # rl_callback_read_char will invoke the lhandler function installed by
  # rl_callback_handler_install to process the line. Before calling the
  # lhandler function, the terminal settings are reset to the values they had
  # before calling rl_callback_handler_install. If the lhandler function
  # returns, and the line handler remains installed, the terminal settings are
  # modified for Readline's use again. EOF is indicated by calling lhandler
  # with a NULL line.
  # 
  ...
  # Restore the terminal to its initial state and remove the line handler. This
  # may be called from within a callback as well as independently. If the
  # lhandler installed by rl_callback_handler_install does not exit the
  # program, either this function or the function referred to by the value of
  # rl_deprep_term_function should be called before the program exits to reset
  ...
  #    If this variable is set to a non-zero value, Readline will install a
  # signal handler for SIGWINCH.
  #
  #    The default value of rl_catch_sigwinch is 1. 
  #
  #Variable: int rl_change_environment
  #    If this variable is set to a non-zero value, and Readline is handling
  ...
  # signals, depending on the values of rl_catch_signals and rl_catch_sigwinch.
  # 
  ...
  # be called before rl_cleanup_after_signal(). The Readline signal handler for
  ...
  # If an application wishes to install its own signal handlers, but still have
  # readline display characters that generate signals, calling this function
  # with sig set to SIGINT, SIGQUIT, or SIGTSTP will display the character
  ...
  # Set Readline's idea of the terminal size to rows rows and cols columns. If
  # either rows or columns is less than or equal to 0, Readline's idea of that
  # terminal dimension is unchanged.
  # 
  ...
  # Return Readline's idea of the terminal's size in the variables pointed to
  # by the arguments.
  # 
  ...
  # SIGALRM, SIGTSTP, SIGTTIN, SIGTTOU, and SIGWINCH, depending on the values
  # of rl_catch_signals and rl_catch_sigwinch.
  # 
  ...
  # This is a pointer to the generator function for rl_completion_matches(). If
  # the value of rl_completion_entry_function is NULL then the default filename
  # generator function, rl_filename_completion_function(), is used. An
  # application-specific completion function is a function whose address is
  # assigned to rl_completion_entry_function and whose return values are used
  # to generate possible completions.
  # 
  # Variable: rl_compentry_func_t * rl_completion_entry_function
  ...
  # Complete the word at or before point. what_to_do says what to do with the
  # completion. A value of `?' means list the possible completions. `TAB' means
  # do standard completion. `*' means insert all of the possible completions.
  # `!' means to display all of the possible completions, if there is more than
  # one, as well as performing partial completion. `@' is similar to `!', but
  # possible completions are not listed if the possible completions share a
  # common prefix. 
  #
  ...
  # Complete the word at or before point. You have supplied the function that
  # does the initial simple matching selection algorithm (see
  # rl_completion_matches() and rl_completion_entry_function). The default is
  # to do filename completion. This calls rl_complete_internal() with an
  ...
  # List the possible completions. See description of rl_complete (). This
  # calls rl_complete_internal() with an argument of `?'. 
  #
  ...
  # partially-completed word. See description of rl_complete(). This calls
  ...
  # Returns the appropriate value to pass to rl_complete_internal() depending
  # on whether cfunc was called twice in succession and the values of the
  # show-all-if-ambiguous and show-all-if-unmodified variables.
  ...
  # array is the substitution for text. The remaining entries are the possible
  # completions. The array is terminated with a NULL pointer.
  #
  #sub char ** rl_completion_matches ( Str $text, rl_compentry_func_t *entry_func)
  #  is native( LIB ) { * }

  # A generator function for filename completion in the general case. text is a
  # partial filename. The Bash source is a useful reference for writing
  # application-specific completion functions (the Bash completion functions
  # call this and other Readline functions). 
  #
  ...
  # A completion generator for usernames. text contains a partial username
  # preceded by a random character (usually `~'). As with all completion
  # generators, state is zero on the first call and non-zero for subsequent
  # calls. 
  #
